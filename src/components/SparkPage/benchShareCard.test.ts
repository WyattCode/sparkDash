...
describe("share card model", () => {
  it("mirrors the decode table: sorted rows, aggregate + stream tok/s, TTFT and streams", () => {
    ...
    expect(card.title).toBe("解码基准测试");
    expect(card.host).toBe("spark-38bd");
    expect(card.subtitle).toBe("端口 8888 · DeepSeek-v4.1-Flash-EXL3");
    expect(card.status).toEqual({ label: "已完成", tone: "ok" });
    expect(card.meta).toBe("散文 · 400 tok · 1, 2 并发 · 27.8 s");
    expect(card.columns).toEqual({ load: "并发", primary: "总吞吐", secondary: "单流" });
    expect(card.rows).toHaveLength(1);
    expect(card.rows[0]).toMatchObject({
      load: "×2",
      detail: "首 Token 399ms · 1/2 流",
      primary: "56.8",
      secondary: "28.6",
      primaryUnit: "tok/s",
      tone: "ok",
  ...
  it("uses prefill context labels and the measured prompt size", () => {
    ...
    expect(card.title).toBe("预填充基准测试");
    expect(card.columns).toEqual({ load: "上下文", primary: "预填充", secondary: "首 Token" });
    expect(card.meta).toBe("1k, 32k 上下文 · 40.0 s");
    expect(card.rows[0]).toMatchObject({
      load: "32k",
      detail: "首 Token 6.64s · 31,998 tokens",
      primary: "4820.5",
      secondary: "6.64s",
      secondaryUnit: "",
    });
  ...
  it("falls back to the port alone when the model is unknown, and to the remote host when used", () => {
    expect(shareCardSubtitle({ llmPort: 8888, modelId: null })).toBe("端口 8888");
  ...
  it("labels every job status", () => {
    expect(shareCardStatus("completed")).toEqual({ label: "已完成", tone: "ok" });
    expect(shareCardStatus("running")).toEqual({ label: "运行中", tone: "warn" });
    expect(shareCardStatus("failed")).toEqual({ label: "失败", tone: "bad" });
    expect(shareCardStatus("cancelled")).toEqual({ label: "已取消", tone: "muted" });
  });
...
describe("share card painter", () => {
  it("draws the header, one line per row, the legend and the footer", () => {
    ...
    expect(ctx.texts).toContain("解码基准测试");
    expect(ctx.texts).toContain("端口 8888 · DeepSeek-v4.1-Flash-EXL3");
    expect(ctx.texts).toContain("已完成");
    expect(ctx.texts).toContain("并发");
    expect(ctx.texts).toContain("总吞吐");
    expect(ctx.texts).toContain("单流");
