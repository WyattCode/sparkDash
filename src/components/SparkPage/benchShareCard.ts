...
export function shareCardSubtitle(src: ShareCardSource): string {
  const target = src.remoteHost ? src.remoteHost : `端口 ${src.llmPort}`;
...
export function buildDecodeShareCard(
  ...
  const rows = job.results
    ...
    .map<ShareCardRow>((r) => {
      ...
      return {
        ...
        detail: failed
          ? r.error || "失败"
          : `首 Token ${formatTtft(r.meanTtftMs)} · ${r.streamsOk}/${streams} 流`,
  ...
  return {
    ...
    meta: `${decodeBenchTypeLabel(job.config?.promptType)} · ${job.config?.maxTokens ?? "?"} tok · ${
      concurrencies.length ? `${concurrencies.join(", ")} 并发` : "—"
    }${job.durationMs != null ? ` · ${formatDuration(job.durationMs)}` : ""}`,
    columns: { load: "并发", primary: "总吞吐", secondary: "单流" },
    rows,
    legend:
      "总吞吐 — 所有并发流的解码总吞吐量。单流 — 每条流的平均解码吞吐量。",
...
export function buildPrefillShareCard(
  ...
  return {
    ...
    subtitle: shareCardSubtitle(src),
    status: shareCardStatus(job.status),
    meta: `${sizes.length ? `${sizes.map(formatContextSize).join(", ")} 上下文` : "—"}${
      job.durationMs != null ? ` · ${formatDuration(job.durationMs)}` : ""
    }`,
    columns: { load: "上下文", primary: "预填充", secondary: "首 Token" },
    rows,
    legend: "预填充 — 提示词 Token 数 ÷ 首 Token 延迟。首 Token — 请求开始到首个 Token 的时间。",
    footer: "github.com/MiaAI-Lab/sparkDash",
    generatedAt: now,
  };
}
