...
export function FleetAlertStrip({ sparks, onSelect }: { sparks: SparkSnapshot[]; onSelect?: (id: string) => void }) {
  ...
  if (alerts.length === 0) return <p className="text-xs text-success" role="status">当前没有集群异常。</p>;
  return (
    <section className="panel p-3" aria-labelledby="fleet-alerts-title">
      <h2 id="fleet-alerts-title" className="text-xs font-semibold text-text-strong">当前集群异常 · {alerts.length}</h2>
