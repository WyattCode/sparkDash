...
it('HTML restores validated theme and density before mounting React',()=>{
  ...
  const run=new Function('localStorage','document',source);
  const doc={documentElement:{dataset:{theme:'dark',density:'compact'}}};
  run({getItem:(key:string)=>key==='sparkdash-theme'?'white':'comfortable'},doc);
  expect(doc.documentElement.dataset).toEqual({theme:'white',density:'comfortable'});
  run({getItem:()=>'<invalid>'},doc);expect(doc.documentElement.dataset.theme).toBe('white');
  expect(()=>run({getItem:()=>{throw Error('disabled');}},doc)).not.toThrow();
